...
<html>
	...
	<%
		List<RecruitInfoDTO> list = (List<RecruitInfoDTO>)request.getAttribute("alist");
	...
	<body>
		...
		<div id="content">
			...
			<table class="t1">
				...
				<tbody>
					<% for(RecruitInfoDTO bean : list) { %>
					<tr>
						<td><%=bean.getIdx()%></td>
						<td id="t1-title">
							<a href = "recruitInfo_detail.hb?idx=<%=bean.getIdx() %>&hit=<%=bean.getHit() %>"><%=bean.getTitle() %></a>
			...
			<div id="con-btn">
				<p>
					<% if(hb_code==5) %><button type="button" OnClick="window.location='recruitInfo_write.hb'">게시물 등록</button>
				</p>
			</div>
			<div class="page">
			<% if(pg.getStartPage() !=1) { 
				int index = pg.getPg() % 5;
				%>	<a href = "recruitInfo_bbs.hb?page=<%=pg.getPg()-index%>"><img class="tag" src = "../imgAll/icon/start.png"></a>
				<% } 
			 %>
			<% for(int i=pg.getStartPage();i<=pg.getEndPage();i++) { %>
				<div><a class="pg-num" href="recruitInfo_bbs.hb?page=<%=i %>"><%=i %></a></div>
			<% } 
			if(pg.getAllpage()>pg.getEndPage()){ %>
				<a href = "recruitInfo_bbs.hb?page=<%=pg.getEndPage()+1%>"><img class="tag" src = "../imgAll/icon/end.png"></a>
			<%} %>
			</div>
		</div>
