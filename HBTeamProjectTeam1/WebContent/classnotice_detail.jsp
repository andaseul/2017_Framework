...
<html>
	...
	<jsp:useBean scope = "request" id = "dto" class = "com.team1.admin.dto.ClassNoticeDTO"></jsp:useBean>
	<body>
		<%@ include file="template/LMS_header.jspf" %>
		<div id="content">
			<div id="con-title">
				<h3>내 강의 > 강의 공지 > 강의 공지 상세</h3>
			</div>
			<table class="t2">
				<thead>
					<tr>
			...
			<div id="con-btn">
				<p>
					<button type="button" OnClick="window.location='classnotice_bbs.hb?page=1'">목록</button>
					<% if(hb_code==3) { %><button type="button" OnClick="window.location='classnotice_edit.hb?idx=<%=dto.getIdx()%>'">수정</button><% } %>
					<% if(hb_code==3) { %><button type="button" OnClick="window.location='classnotice_del.hb?idx=<%=dto.getIdx()%>'">삭제</button><% } %>
				</p>
			</div>
		</div><!-- end content -->
